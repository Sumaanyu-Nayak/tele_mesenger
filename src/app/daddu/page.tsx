...
export default function Home() {
  ...
  async function fetchMessages() {
    setFetching(true);
    const res = await fetch("/api/messages-daddu");
          ...
          {[...messages].reverse().map((msg) => {
            ...
            return (
              <div className={styles.chatMsg} key={msg._id} style={{ justifyContent: isSent ? 'flex-end' : 'flex-start' }}>
                {!isSent && (
                  <div className={styles.avatar} title={msg.message?.from?.username || 'Daddu'}>
                ...
                <div className={`${styles.bubble} ${isSent ? styles.sent : styles.received}`}>
                  ...
                  <div className={styles.msgMeta}>
                    <span className={styles.msgSender}>
                      {isSent ? 'You' : (msg.message?.from?.username ? `@${msg.message.from.username}` : 'Daddu')}
                    </span>
